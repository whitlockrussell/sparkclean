...
export async function generateTaxSummaryPDF(d: TaxSummaryPDFData): Promise<void> {
  ...
  let metaY = y + 11
  if (d.business.hstNumber) { doc.text(`${d.business.taxNumberLabel ?? 'Tax #'}: ${d.business.hstNumber}`, nameX, metaY); metaY += 4.5 }
  ...
  doc.text('Subtotal from invoices marked paid.', ML, y)
  y += 8
  ...
  if (d.mileage.km > 0) {
    ...
    y = row(doc, 'Standard rate (2025, first 5,000 km)', '$0.72/km', y)
  ...
  y = row(doc, 'Revenue', fmt(d.revenue.totalPaid), y)
  ...
  const disclaimer = 'This summary reflects income and expenses tracked in SparkClean only. SparkClean does not provide tax or financial advice. Consult a qualified tax professional for guidance on your specific situation.'
